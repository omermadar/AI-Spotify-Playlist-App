#!/usr/bin/env python3
"""
AI Music Recommendation System with Spotify Integration
Interactive playlist creation from natural language descriptions
"""
...
from MusicRecommender import MusicRecommender  # Use your fixed version
from PlaylistCreator import PlaylistCreator  # Use your fixed version

# Import Spotipy directly instead of broken integration
import spotipy
from spotipy.oauth2 import SpotifyOAuth


class WorkingSpotifyIntegration:
    """
    Working Spotify integration using manual URL authentication.
    This method always works regardless of browser/environment issues.
    """
    ...
    def authenticate(self) -> bool:
        ...
        try:
            ...
            print("📄 After authorization, you'll see a page (might show an error - that's normal)")
    ...
    def create_playlist_from_songs(self, playlist_name: str, description: str, songs: List[Dict]) -> Dict:
        ...
        try:
            ...
            if track_uris:
                ...
                if not_found:
                    print(f"\n⚠️  Could not find {len(not_found)} songs:")
                    for song in not_found[:3]:
                        print(f"  - {song}")
                    if len(not_found) > 3:
...
class MusicPlaylistApp:
    ...
    def setup_spotify(self) -> bool:
        ...
        if self.SPOTIFY_CLIENT_ID == "your_client_id_here":
            return self.setup_spotify_credentials()

        try:
            print("\n🎵 Connecting to Spotify...")

            # Use the WORKING integration class
            self.spotify = WorkingSpotifyIntegration(
    ...
    def create_spotify_playlist(self, playlist_data: dict, custom_name: str = None) -> bool:
        ...
        try:
            ...
            print(f"\n🎵 Creating Spotify playlist: '{name}'")
